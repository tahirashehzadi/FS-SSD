...
def apply_attention_blocks(x):
        orig_x = x
        num_temporal_attention_blocks = 8
        batch_size, roi_h, roi_w = x.size(0), x.size(1), x.size(2)

        x = x.view(batch_size, 1, 1, roi_h, roi_w)
        quarter_scale_x = x.clone()  # Create a copy of the input tensor for quarter_scale_x

        x = torch.cat((x, quarter_scale_x), dim=2)
        batch_size, img_n, _, roi_h, roi_w = x.size()

        # Calculating the number of attention blocks
        num_attention_blocks = num_temporal_attention_blocks
        x_embed = x
        c_embed = x_embed.size(2)

        # Performing multi-head attention
        x_embed = x_embed.view(batch_size, img_n, num_attention_blocks, -1, roi_h, roi_w)
        target_x_embed = x_embed[:, [1]]
        ada_weights = torch.sum(x_embed * target_x_embed, dim=3, keepdim=True) / (float(c_embed / num_attention_blocks)**0.5)
        ada_weights = ada_weights.expand(-1, -1, -1, int(c_embed / num_attention_blocks), -1, -1).contiguous()
