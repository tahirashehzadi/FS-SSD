...
from detr_ssod.apis.inference import init_detector
...
class Projector2(nn.Module):
    def __init__(self, l1=1, l2=1, s=16):
        super().__init__()
        
    
        self.m1 = nn.Linear(l2 * s, l1 * s).to('cuda')
        
        
    def forward(self, x2_flattened):
        
        
        x2_flattened = self.m1(x2_flattened)
        return x2_flattened
       
        
        
class Projector3(nn.Module):
    def __init__(self, l1=1, l2=1, s=16, s1=256, s2=16):
        super().__init__()
        
    
        self.m11 = nn.Linear(l2 * s, l1 * s).to('cuda')
        
        self.linear_d = nn.Linear(s1, s2).to('cuda')
        self.linear_i = nn.Linear(s2, s1).to('cuda')
...
class DinoDetrSSOD(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        ...
        #teacher 2 trained on 10% of coco
        ...
        self.label_map = {i : class_name for i, class_name in enumerate(CLASSES)}        
    ...
    def compute_pseudo_label_loss(self, student_info, teacher_info):
        ...
        pseudo_bboxes1 = self._transform_bbox(
            teacher_info["det_bboxes"],             # 
            M,
            [meta["img_shape"] for meta in student_info["img_metas"]],
        )
        # import ipdb;ipdb.set_trace()
        pseudo_labels1 = teacher_info["det_labels"]
        pseudo_scores1 = teacher_info["det_scores"]


        pseudo_bboxes = pseudo_bboxes1
        pseudo_labels = pseudo_labels1
        pseudo_scores = pseudo_scores1
        loss = {}
    ...
    def unsup_loss(
        ...
        **kwargs,
        ...
        targets_v1['scores'] = gt_scores_list
        ...
        targets_v2['scores'] = det_scores_gmm_list
        # becareful when prepare the dn_components, the gt bboxes needs to 
        # be normalized with corresponding w, h, in the cx, cy, w, h format
        normalized_boxes = []
    ...
    def prepare_unsup_cdn(self, teacher_info, student_info, pseudo_bboxes, pseudo_labels, det_bboxes, det_laebls, 
        ...
        for img_meta, pseudo_bbox in zip(img_metas_tgt, pseudo_bboxes):
            ...
            transform_matrix = torch.from_numpy(img_meta['transform_matrix']).float().to(pseudo_bbox.device)
            inv_transform_matrix = torch.inverse(transform_matrix)
            pseudo_bbox_orig = Transform2D.transform_bboxes(pseudo_bbox, inv_transform_matrix, (img_h, img_w))
            
            for idx, bbox in enumerate(pseudo_bbox_orig):
                ...
                if crop_width > 0 and crop_height > 0:
                    ...
                    #print(f"Skipping crop: Invalid crop size (width={crop_width}, height={crop_height}), using full image.")
                    roi_image = image 
                    
                if roi_image.mode not in ("RGB", "L"):  
                    roi_image = roi_image.convert("RGB")
                 

                image_input = self.t3_preprocess(roi_image).unsqueeze(0).to(imgs_tgt.device)  # Shape: (1, 3, H, W)
                text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in self.label_map.values()]).to(imgs_tgt.device)
                 
                with torch.no_grad():
                  image_features = self.teacher3.encode_image(image_input)
                  text_features = self.teacher3.encode_text(text_inputs)
                ...
                query_text_labels_list.extend(indices.tolist())"""
                


        
        # ++++++ consistency part ++++++ #
        for img_meta, pseudo_bbox, pseudo_label in zip(img_metas_tgt, pseudo_bboxes, pseudo_labels):
            img_h, img_w, _ = img_meta['img_shape']
            # print(t2_det_labels, t2_det_scores)
            # if there exist some empty pseudo bbox
            if pseudo_bbox.size(0) == 0:
                pseudo_bbox = pseudo_bbox.new_tensor([[img_w / 4, img_h / 4, 3 * img_w/ 4, 3 * img_h / 4]])
                pseudo_label = pseudo_label.new_tensor([0])
                
            query_text_labels_list.append(pseudo_label)
        ...
        # ========== query text part ========== #
        #print("all_top_indices",len(query_text_labels_list))
        #print("batched_tgt_bboxes",len(batched_tgt_bboxes))  
        #query_text_labels = torch.tensor(query_text_labels_list)
        #print("query_text_labels_list",query_text_labels_list)
        query_text_labels = torch.cat(query_text_labels_list)
        ...
        single_pad_3 = int(max(known_num))
        pad_size_3 = int(single_pad_3 * dn_number_1)     
        ...
        input_query_label_3 = padding_label.repeat(batch_size, 1, 1)
        input_query_bbox_3 = padding_bbox.repeat(batch_size, 1, 1)
        
        # bbox embed
        map_known_indice_1 = torch.tensor([]).to('cuda')
        if len(known_num):
            map_known_indice_1 = torch.cat([torch.tensor(range(num)) for num in known_num])
            map_known_indice_1 = torch.cat([map_known_indice_1 + single_pad_3 * i for i in range(dn_number_1)]).long()

        input_query_bbox_3[(known_bid_1.long(), map_known_indice_1)] = consistency_bbox_embed
        
        known_labels = query_text_labels.repeat(dn_number_1, 1).view(-1)
        known_labels_expaned = known_labels.clone()
        m = known_labels_expaned.long().to('cuda')
        input_label_embed = self.student.bbox_head.label_enc(m)
        input_query_label_3[(known_bid_1.long(), map_known_indice_1)] = input_label_embed
        
        #print("input_query_bbox_3",input_query_bbox_3.size())
        #print("input_query_label_3",input_query_label_3.size())
        ...
        scores = torch.cat([t for t in gt_scores])
        ...
        known_scores = scores.repeat(2 * dn_number_2, 1).view(-1)
        known_scores_expaned = known_scores.clone()
        ...
        random_labels = torch.randint(0, num_classes, (len(negative_idx),), device=known_labels_expaned.device)
        known_labels_expaned[negative_idx] = random_labels
        ...
        label_vector2 = self.enhanced_representation_label(input_query_label_3, input_query_label_2)
        input_vector2 = self.enhanced_representation(input_query_bbox_3, input_query_bbox_2)
  
        
        
        input_query_label_2= torch.cat([input_query_label_3, label_vector2], dim=1)
        input_query_bbox_2 = torch.cat([input_query_bbox_3, input_vector2], dim=1)
        
        pad_size_22 = input_query_label_2.size(1)
        #print("pad_size_22",pad_size_22)
        #print("pad_size_2",pad_size_2)
        tgt_size = pad_size_1 + pad_size_22 + num_queries
    ...
    def extract_teacher_info(self, img, img_metas,teacher="teacher", **kwargs):
        ...
        proposal_list = model.bbox_head.simple_test_bboxes(
    ...
    def enhanced_representation(self, label_1, label_2):
        ...
        dot_product = torch.bmm(label_1, label_2.transpose(1, 2))
        norm_a = label_1.norm(dim=2, keepdim=True)
        norm_b = label_2.norm(dim=2, keepdim=True)
        similarity = dot_product / (norm_a * norm_b.transpose(1, 2))

       
        max_similar_indices = torch.argmax(similarity, dim=2)
        similar_vectors = torch.zeros_like(label_1)
        
                
        for i in range(similar_vectors.size(0)):  # Looping over the first dimension (size 4)
            for j in range(similar_vectors.size(1)):  # Looping over the second dimension (size 200)
                if j < max_similar_indices.size(1):  # If j is less than 80
                   clamped_index = max_similar_indices[i, j].clamp(0, label_1.size(1) - 1)
                   similar_vectors[i, j] = label_2[i, clamped_index]
        label_2 = similar_vectors
        query = label_2
        key = label_2
        value = label_2
        ...
        e_x = torch.exp(scores - torch.max(scores, dim=-1, keepdim=True).values)
        attention_weights = e_x / e_x.sum(dim=-1, keepdim=True)
        ...
        num_temporal_attention_blocks = 1
        ...
        #print("Size of orig_x1:", orig_x1.shape)
        #print("Size of x_upsampled:", x_upsampled.shape)
        aggregated_enhanced_representation = orig_x1 + x_upsampled
        
        #projector_instance = Projector2( l1=x1.size(1), l2=x2.size(1), s=x1.size(2))
        #aggregated_enhanced_representation= projector_instance.last(aggregated_enhanced_representation)
        
        return aggregated_enhanced_representation
    
    def enhanced_representation_label(self, label_1, label_2):

        x1 = label_1
        x2 = label_2
        batch_size, l1, s = x1.size(0), x1.size(1), x1.size(2)
        batch_size, l2, s = x2.size(0), x2.size(1), x2.size(2)
        projector_instance3 = Projector3( l1=x1.size(1), l2=x2.size(1), s=16, s1=s, s2=16)
        s2=16
       
        
        
        x1 = x1.view(-1, x1.size(2))
        x1= projector_instance3.linear_d(x1)
        x1 = x1.view(batch_size,l1,s2 )
        #print("Size of x1:", x1.shape)
        
        x2 = x2.view(-1, x2.size(2))
        x2= projector_instance3.linear_d(x2)
        x2 = x2.view(batch_size,l2, s2)
        ...
        l_2=x2
        l_1= x1
        #print("Size of label_1:", label_1.shape)
        #print("Size of label_2:", label_2.shape)
        dot_product = torch.bmm(l_1, l_2.transpose(1, 2))
        norm_a = l_1.norm(dim=2, keepdim=True)
        norm_b = l_2.norm(dim=2, keepdim=True)
        similarity = dot_product / (norm_a * norm_b.transpose(1, 2))

        #print("Size of label_1:", label_1.shape)
        #print("Size of label_2:", label_2.shape)
        max_similar_indices = torch.argmax(similarity, dim=2)
        similar_vectors = torch.zeros_like(l_1)
        
         
        for i in range(similar_vectors.size(0)):  # Looping over the first dimension (size 4)
            for j in range(similar_vectors.size(1)):  # Looping over the second dimension (size 200)
                if j < max_similar_indices.size(1):  # If j is less than 80
                   clamped_index = max_similar_indices[i, j].clamp(0, l_1.size(1) - 1)
                   similar_vectors[i, j] = l_2[i, clamped_index]
        xx2= similar_vectors
        query = xx2
        key = xx2
        value = xx2
        ...
        #print("Size of x2_flattened:", x2_flattened.shape)
        #x2_transformed= projector_instance3.m11(x2_flattened)
        #x2_final = x2_transformed.view(batch_size, l1, s2)
        #print("Size of x1 cat:", x1.shape)
        #print("Size of x2:", x2_final.shape)
        #x = torch.cat((x1, x2_final), dim=1)  
        ...
        f = aggregated_enhanced_representation.view(-1, aggregated_enhanced_representation.size(2))
        f= projector_instance3.linear_i(f)
        f = f.view(aggregated_enhanced_representation.size(0),aggregated_enhanced_representation.size(1),256 )
