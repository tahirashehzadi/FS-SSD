...
def imshow_det_bboxes(img,
    ...
    if label_color:
        # make different label with different color
        color_list = ["red", "green", "blue", "cyan", "yellow", "magenta", "white", "black"]
        label_val = np.unique(labels)
        assert len(label_val) <= len(color_list), "uniquel label set is larger than the color set!"
        bbox_color = []
        text_color = []
        for color in color_list:
            bbox_color.append(color_val_matplotlib(color))
            text_color.append(color_val_matplotlib(color))
    else:
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        ax.text(
            ...
            color=text_color[label] if label_color else text_color,
            fontsize=font_size,
